 ...
 * @param old Old motor state
 * @param new New motor state
*/
...
static inline int8_t get_pwm(int32_t distance) {
	distance = max(min(distance, 200), -200);
	int8_t pwm = min(max(((distance * abs(distance)) >> 8), -100), 100);
	if (distance < 0 && pwm > -MIN_PWM_DUTY) {
		return -MIN_PWM_DUTY;
	} else if (distance > 0 && pwm < MIN_PWM_DUTY) {
		return MIN_PWM_DUTY;
	} else {
		return pwm;
	}
...
void motor_isr(void) {
    ...
    if (set_position_to_desired) {
        duty = get_pwm(motor_driver_shared.desired_position - motor_driver_shared.position);
    	set_motor_dutycycle(duty);
    	motor_driver_shared.pwm_duty = duty;
    }
...
void motor(bool set_desired) {
    ...
    while (1) {
        ...
        if (set_desired) {
        	...
        	int8_t duty = get_pwm(motor_driver_shared.desired_position - motor_driver_shared.position);
            intDisable(INT_LVL_GPIO);
            set_motor_dutycycle(duty);
            motor_driver_shared.pwm_duty = duty;
